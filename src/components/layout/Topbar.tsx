  ...
  return (
    ...
    <header className="no-print glass top-3 z-40 flex flex-wrap items-center justify-between gap-4 rounded-3xl px-4 py-3 lg:sticky">
      <Link href="/" className="flex items-baseline gap-1.5">
        <span className="text-2xl font-bold tracking-tight text-slate-900">projex</span>
      ...
      <div className="order-3 flex w-full items-center gap-1.5 overflow-x-auto lg:order-none lg:w-auto">
          ...
          <button
            ...
            className={cn(
              ...
              range === r
                ...
                : "bg-white/50 text-slate-600 hover:bg-white/90"
            )}
          >
            {RANGE_LABEL[r]}
          </button>
        ...
        <Link
          href="/report"
          className="shrink-0 rounded-full bg-white/50 px-4 py-2 text-sm font-medium text-slate-600 transition hover:bg-white/90"
        >
          Laporan
        </Link>
      </div>
      ...
      <div className="flex items-center gap-2">
        <Link
          ...
          aria-label="Laporan"
          title="Laporan"
          className="grid size-10 place-items-center rounded-full bg-white/50 text-slate-500 transition hover:bg-white/90 hover:text-slate-900"
        >
        ...
        <div ref={menuRef} className="relative">
          <button
            ...
            aria-haspopup="menu"
            className="flex items-center gap-2.5 rounded-full bg-white/50 py-1.5 pr-3 pl-1.5 transition hover:bg-white/90"
          >
            {currentMember ? (
            ...
            >
              ...
              {members.map((m) => (
                <button
                  ...
                  className={cn(
                    "flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition hover:bg-white/80",
                    currentMember?.id === m.id && "bg-white/80"
                  )}
                >
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block truncate text-xs text-slate-500">{m.role}</span>
                  </span>
                </button>
              ))}

              <div className="mt-2 border-t border-slate-100 px-3 pt-2 pb-1">
                <span
                  className={cn(
                    "text-xs font-medium",
